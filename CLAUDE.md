...
Default font is **Monaco 12pt** (same as TextEdit), with fallback to Menlo 13pt if Monaco is not available. Font size is persisted in `UserDefaults` under key `"fontSize"` and read on startup by `EditorView.setupTextView()`. Zoom actions (`zoomIn`, `zoomOut`, `resetZoom`) live on `EditorViewController` and update `textView.font` directly.
...
- `StatusBarView` (defined in `EditorView.swift`) — thin 22pt bar anchored to the bottom of `EditorView`, above which both the ruler and scroll view terminate. Contains an `NSBox` separator at its top edge and a right-aligned `NSTextField` label. Updated via `EditorView.updateStatus(line:col:words:chars:)`. Uses semantic colors (`secondaryLabelColor`) so it adapts to dark/light mode automatically.
...
- `HighlightTheme.swift` — Maps tree-sitter capture names (e.g. `"keyword"`, `"string"`, `"comment"`) to VS Code Dark+/Light+ colors using the same `vscodeColor(dark:light:)` helper pattern. Uses prefix matching so sub-scopes like `"keyword.control"` correctly resolve to the `"keyword"` color. Called by `HighlightCoordinator`'s `TokenAttributeProvider`.
