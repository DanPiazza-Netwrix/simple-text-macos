...
- `LineNumberRulerView.swift` — Plain `NSView` subclass (NOT `NSRulerView`). Positioned as a sibling to the scroll view inside `EditorView`, avoiding all `NSScrollView` ruler machinery (separator lines, dividers). `isFlipped = true` so coordinate math works the same as scroll view content. Uses `NSLayoutManager.enumerateLineFragments` to position numbers. Coordinate math: `rulerY = fragmentRect.midY + textContainerOrigin.y - clipView.bounds.origin.y`. Width is managed via an `NSLayoutConstraint` updated by `updateWidthIfNeeded()`, which is called from the `NSTextStorage.didProcessEditingNotification` observer — **never from `draw()`**. Mutating a constraint inside `draw()` triggers a layout pass that resizes the scroll view → text view → text container, which corrupts NSTextView's cursor state mid-operation and can cause phantom deletions. The visible-rect query is expanded by ±4pt so `glyphRange(forBoundingRect:)` doesn't clip the last line fragment at the boundary.
...
- `EditorViewController.swift` — central coordinator; owns `DocumentController`, `AppearanceManager`, `EditorView`. All `@objc` menu actions wired via responder chain (`target: nil` in `AppDelegate.buildMainMenu`). Exposes `onFilesDropped: (([URL]) -> Void)?` callback forwarded from `EditorView` up to `TabController`. Owns a per-tab `tabUndoManager: UndoManager` (returned by `WindowController.windowWillReturnUndoManager`). Conforms to `NSTextViewDelegate` and calls `textView.breakUndoCoalescing()` for single-character word-boundary typing (space, newline, punctuation) — guarded to `string.count == 1` so paste/multi-char operations are excluded (they corrupt NSTextView's undo group state), and skipped during programmatic `insertText` calls via `EditorTextView.isProgrammaticTextChange`. Uses `applyHighlighting(for:)` to pick between `SyntaxHighlighter` (Markdown) and `HighlightCoordinator` (everything else via `LanguageRegistry`); called from both `documentDidLoad` and `documentDidSave` so that saving an untitled buffer under a new name (e.g. `test.ps1`) activates highlighting immediately without requiring a reload. Font zoom actions (`zoomIn`, `zoomOut`, `resetZoom`) update `textView.font` and persist size to `UserDefaults`. `updateStatusBar()` is called from `selectionDidChange`, `handleTextDidChange`, and `documentDidLoad` to keep the status bar current.
...
- `EditorViewController` owns a `let tabUndoManager = UndoManager()` instance. **Do NOT set `groupsByEvent = false`** — that breaks typing entirely because NSTextView requires `groupsByEvent = true` (the default) for its own undo group management to function.
...
- Word-boundary granularity: `EditorViewController` conforms to `NSTextViewDelegate` and calls `textView.breakUndoCoalescing()` for **single-character** typing at word boundaries (space, newline, punctuation). This seals the previous word as its own undo step. **Critical constraints:** (1) Only fires when `string.count == 1` — paste/autocorrect/multi-char replacements are excluded because calling `breakUndoCoalescing()` before a multi-character insertion corrupts NSTextView's internal undo group state and causes phantom deletions on subsequent edits. (2) Skipped when `EditorTextView.isProgrammaticTextChange` is `true` (set by `indentSelectedLines`/`dedentSelectedLines` around their `insertText` call).
...
**Do NOT** try to manage undo grouping manually via `beginUndoGrouping`/`endUndoGrouping`. With `groupsByEvent = true`, NSTextView manages all grouping itself. Never set `groupsByEvent = false` on `tabUndoManager` — doing so breaks typing entirely.

**Indent/dedent selection restoration:** `lineRange(for:)` always includes the trailing `\n` in the returned range. After `indentSelectedLines`/`dedentSelectedLines`, the restored selection is trimmed to exclude that trailing newline if the user's original selection did not include it (detected by `lineRange.length > sel.length`). This prevents pressing Delete immediately after Tab from deleting the line terminator and unexpectedly merging the following line.
